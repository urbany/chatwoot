...
RSpec.describe Webhooks::WhatsappEventsJob do
  ...
  context 'when whatsapp business params' do
    ...
    it 'stores a contact reaction on the referenced message without creating a new message' do
      other_channel = create(:channel_whatsapp, phone_number: '+1987654', provider: 'whatsapp_cloud', sync_templates: false,
                                                validate_provider_config: false)
      contact = create(:contact, account: other_channel.account, name: 'Test Test')
      contact_inbox = create(:contact_inbox, contact: contact, inbox: other_channel.inbox, source_id: '1111981136571')
      conversation = create(:conversation, account: other_channel.account, inbox: other_channel.inbox, contact: contact,
                                           contact_inbox: contact_inbox)
      target_message = create(:message, account: other_channel.account, inbox: other_channel.inbox, conversation: conversation,
                                        sender: create(:user, account: other_channel.account), message_type: 'outgoing',
                                        source_id: 'wamid.target.message')
      wb_params = {
        phone_number: channel.phone_number,
        object: 'whatsapp_business_account',
        entry: [{
      ...
      reactions = target_message.reload.content_attributes['reactions']
      expect(reactions).to be_present
      expect(reactions['contact']).to include(
        'actor_type' => 'contact',
        'actor_id' => contact.id,
        'actor_name' => 'Test Test',
        'emoji' => '👍',
        'wa_id' => '1111981136571',
        'reaction_source_id' => 'wamid.reaction.message'
      )
    end

    it 'does not create a contact if a reaction arrives before the referenced target exists' do
      other_channel = create(:channel_whatsapp, phone_number: '+1987654', provider: 'whatsapp_cloud', sync_templates: false,
                                                validate_provider_config: false)
      wb_params = {
        phone_number: channel.phone_number,
          ...
          changes: [{
            value: {
              ...
              messages: [{
                id: 'wamid.reaction.message',
                from: '1111981136571',
                reaction: { message_id: 'wamid.missing.target', emoji: '👍' },
                timestamp: '1664799904',
                type: 'reaction'
              }],
              metadata: {
    ...
    it 'removes the stored contact reaction when the webhook emoji is blank' do
      other_channel = create(:channel_whatsapp, phone_number: '+1987654', provider: 'whatsapp_cloud', sync_templates: false,
                                                validate_provider_config: false)
      contact = create(:contact, account: other_channel.account, name: 'Test Test')
      contact_inbox = create(:contact_inbox, contact: contact, inbox: other_channel.inbox, source_id: '1111981136571')
      ...
      expect(target_message.reload.content_attributes['reactions']).to be_blank
    end
