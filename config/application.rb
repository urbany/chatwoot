...
module Chatwoot
  class Application < Rails::Application
    ...
    # `custom/` mirrors the `enterprise/` extension namespace (see ChatwootApp.extensions
    # and config/initializers/01_inject_enterprise_edition_module.rb) for fork-local
    # patches that use `prepend_mod_with`/`include_mod_with` instead of editing OSS files.
    if Rails.root.join('custom').exist?
      # rubocop:disable Rails/FilePath
      config.eager_load_paths += Dir["#{Rails.root}/custom/app/**"]
      # rubocop:enable Rails/FilePath
      # Add custom views to the view paths
      config.paths['app/views'].unshift('custom/app/views') if Rails.root.join('custom/app/views').exist?

      # Load custom initializers alongside standard initializers
